...
@section("content")
 ...
 <div class="row my-4">
    <form action="{{url('users')}}/{{$user->id}}" method="post" class="card border-0 shadow p-3 pb-4 mb-4">
        @csrf
           ...
           <div class="row justify-content-center">
            <div class="col-12 col-lg-6">
                <div class="form-group mb-4"><label for="cartInputCompany1">User Name *</label>
                     <input type="text" placeholder="Name" value="{{old('username', $user->username)}}" name="username" class="form-control"  required></div>
             </div>
              <div class="col-12 col-lg-6">
                 <div class="form-group mb-4"><label for="cartInputEmail1">Email address *</label>
                     <input type="email" class="form-control" name="email" value="{{old('email', $user->email)}}" placeholder="example@company.com"  required>
                     {!! $errors->first('email', '<span class="text-danger">:message</span>') !!}
                  </div>
              </div>
              <div class="col-12 col-lg-6">
                <div class="form-group mb-4"><label for="cartInputEmail1">Password *</label>
                    <input type="password" class="form-control" name="password" placeholder="Leave empty if you do not want to chane password"></div>
             </div>
             <div class="col-12 col-lg-6">
                <div class="form-group mb-4"><label for="cartInputEmail1">Confirm Password *</label>
                    <input type="password" class="form-control" name="password2" placeholder="Leave empty if you do not want to chane password"></div>
             </div>
        ...
        <div class="card-body p-0 p-md-4">
            <div class="row justify-content-center">
                            ...
                            @foreach($modules as $module)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{ in_array(1, array_merge(old($module->id . 'permission', []), \App\Models\UserPermissionModel::getModulePermissions($module->id, $user))) ? 'checked' : '' }}>
                                    </td>
                                    <td>
                                        <input class="form-check-input" type="checkbox" value="2" name="{{$module->id . 'permission[]'}}" 
                                        {{ in_array(2, array_merge(old($module->id . 'permission', []), \App\Models\UserPermissionModel::getModulePermissions($module->id, $user))) ? 'checked' : '' }}>
                                    </td>
                                    <td>
                                        <input class="form-check-input" type="checkbox" value="3" name="{{$module->id . 'permission[]'}}" 
                                        {{ in_array(3, array_merge(old($module->id . 'permission', []), \App\Models\UserPermissionModel::getModulePermissions($module->id, $user))) ? 'checked' : '' }}>
                                    </td>
                                </tr>
                            @endforeach
               ...
               <div class="col-12"><button class="btn btn-gray-800 mt-5 animate-up-2" type="submit">Edit User</button></div>
            </div>
         </div>
     </form>
</div>
