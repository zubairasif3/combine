    ...
    <div class="py-4">
        ...
        <div class="d-flex justify-content-between w-100 flex-wrap">
           <div class="mb-3 mb-lg-0">
            <h1 class="h4">Send Payments List</h1>
            <p class="mb-0">List of payments in our system.</p>
           </div>
        </div>
     </div>
                  ...
                  <tr>
                     ...
                     @if ($job->payment !== null)
                        ...
                        <td class=" text-gray-900">
                           <span class="badge super-badge bg-{{$job->payment->status == "sent" ? 'success' : 'info'}} ms-1">{{$job->payment->status}}</span>
                        </td>
                        ...
                        <td>
                           @if ($job->payment->status == "sent")
                              <a href="{{url('payments/received').'/'.$job->payment->id}}" class="btn btn btn-outline-tertiary action-btn d-inline-flex align-items-center">
                     ...
                     @else
                        ...
                        <td class=" text-gray-900">
                            @if ($job->contract_status != '0')
                                <span class="badge super-badge bg-{{$job->contract_status === '0' ? 'success' : 'danger'}} ms-1">{{$job->contract_status === '0' ? 'Accept' : 'Reject'}}</span>
                            @else
                                <button onclick="executeAccept({{$job->id}})" href="button" class="btn btn btn-outline-success action-btn d-inline-flex align-items-center" data-bs-toggle="modal" data-bs-target="#modal-accept">Accept</button>
                                <a href="{{ route('job.reject', $job->id)}}" class="btn btn btn-outline-danger action-btn d-inline-flex align-items-center" >
                                    Reject
                        ...
                        <td>
                           <a href="{{url('payments/sent').'/'.$job->id}}" class="btn btn-outline-success action-btn d-inline-flex align-items-center">
                              <svg class="icon icon-xxs me-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
